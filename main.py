...
def run(get_dataset, num_iterations, set_size, alphas, utility_sampling_mode,
    ...
    for t in range(num_iterations):
        ...
        # dataset, not the ones generated by the solver) according to the
        # hidden user vector
        hidden_scores = np.dot(hidden_w, best_items.T)
        avg_losses.append(best_latent_score - np.mean(hidden_scores))
