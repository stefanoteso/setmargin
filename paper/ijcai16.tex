...
\begin{itemize}
 \item The Bayesian approach from \cite{guo2010real}, selecting queries according to {\em restricted uninformed VOI} ({\sc ruVOI}), an heuristic approximation of value-of-information, and performing inference using  TrueSkill$^{TM}$ \cite{HerbrichMG06} (an inference scheme based on expectation propagation \cite{Minka01}).
...
\begin{figure*}[h!]
    ...
    \caption{\label{fig:comparison} Comparison between {\sc SetMargin}
      (orange), {\sc ruVOI} (blue), {\sc QI} (green) and {\sc EUS}
...
terms of cumulative time. Given that {\sc ruVOI},  {\sc QI} and
...
algorithm for $k=2$ is reported in orange, {\sc ruVOI} in blue, {\sc QI}
...
%cannot scale over $r=4$ (for $r=5$, {\sc ruVOI} did not
...
% {\sc ruVOI} e' piantato al primo utente da tre giorni su synthetic 5.
