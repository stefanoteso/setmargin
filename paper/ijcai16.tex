...
depend entirely on the former. For instance, the price, weight and power
consumption of a laptop may be modelled as real-valued attributes, and further
they depend entirely on the choice of components.
%
In this setting, item configurations are composed of two parts: $\vx = (\vx_B;\vx_R)$, where $\vx_B$ is
Boolean and $\vx_R$ is real-valued, and the relation between the two is
entirely described by a non-negative\footnote{Non-negativity is required for
the MILP translation to work.\stefano{better explanation please}} cost matrix
$C\in\bbR^{\text{num costs} \times \text{num bools}}$, such that $\vx_R = C
\vx_B$.  It is straightforward to extend the MILP formulation to this setting,
as follows.

First, we rewrite the weight vector as $\vw = (\vw_B;\vw_R)$. The utility becomes
%
$$ \langle \vw, \vx \rangle = \langle \vw_B, \vx_B \rangle + \langle \vw_R, C \vx_B \rangle = \langle \vw_B + C^\top \vw_B, \vx_B \rangle $$
%
We let $\vv^i := \vw_B^i + C^\top \vv_B^i$, and $q^{ij}_z := (\vw^i_B + C^\top \vw^i_R)_z x^j_z$,
and substitute them in the MILP problem to $\vw$ and $p^{ij}_z$, respectively.
