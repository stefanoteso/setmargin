...
\begin{itemize}
    ...
    \item \ref{fig:comparison} setmargin vs Bayes. similar loss wrt Bayes.
    variance slighly higher on synthetic 4. funzioniamo meglio su sparso al
    contrario degli altri. Guo e' piantato al primo utente da tre giorni
    su synthetic 5.  Paolo procede su S-5 da qualche ora (~8) (1290s su
    synthetic-5 uniforme per completamento di un utente).

    \stefano{@Paolo: varianza tempi?}

    \item \ref{fig:selfcomparison} crossval ogni 5 iterazioni, il che vuol
    dire diverso numero di queri per $k$ diversi.
