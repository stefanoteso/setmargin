...
\newcommand{\diff}[1]{{\bf \textcolor{violet}{#1}}}
...
variables $\mu \in \bbR_{\ge 0}$, $\{ \vw^i \in \bbR^m \}$, $\{ \vx^i \in \{0,1\}^m \}$ \stefano{stress non-zero margin requirement}:
%
{\footnotesize
\begin{align}
    \max
...
a suitable variable transformation; \stefano{this technique has already been
used in OR~[Boutilier et al., 2006]}.
...
x^j_z$.\footnote{\stefano{This is only relevant for $k=1$.} Since $\mu$ is upper-bounded by Eq.~(\ref{eq:wyconstr}), in some
