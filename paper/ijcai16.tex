...
The $\ell_1$ vector norm is denoted as $\|\vx\|_1 := \sum_z |x_z|$.
...
Python, and leverage Gurobi 6.5.0 for solving the MILP problem. Both the 
{\sc SetMargin} source code and the full experimental setup are available at
\stefano{add URL}.
...
%
$$ P(\vx^i > \vx^j \;|\; \vw, \vx^i \not\sim \vx^j) = (1 + \exp(-\alpha \langle\vw,\vx^i - \vx^j\rangle))^{-1} $$
%
...
%
$$ P(\vx^i \sim \vx^j \;|\; \vw) = \exp(\beta |\langle\vw,\vx^i - \vx^j\rangle|) $$
%
...
$80\%$ uniformly at random \stefano{Paolo, double check please} the entries
from the vectors sampled in the uniform and normal settings. We sampled $100$
