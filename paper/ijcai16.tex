...
continuous integer variables. Here we show how to reformulate the
...
simulatins, as in~\cite{guo2010real}. Please see the latter for more
details. \stefano{The above does not satisfy me at all.}\andrea{sounds reasonable now, I would remove Please see..?}
...
\stefano{explain that we can't turn off $\gamma$}\andrea{not so needed, but explain why ranges are different}
...
of configurations, for $r=2$ the synthetic dataset corresponds to 
$\calX_\text{feasible} = [2] \times [2]$, for $r=3$ to
...
variance. \andrea{standard deviation?}
...
\stefano{too detailed/handwavy?} \andrea{si'. irrilevante}
...
into MILP form as:\andrea{sbaglio o e' vecchiotto il dataset? footnote?}
%
...
size, type $\to$ monitor size. In total we encoded 16 Horn
constraints, each with one head and multiple options in the body. We
