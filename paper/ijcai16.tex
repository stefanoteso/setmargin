...
will be indifferent to most of them. %~\cite{} \stefano{@Paolo: add refs}. 
...
\item The best Bayesian approach from \cite{guo2010real}, selecting
  ...
  heuristic approximation of value-of-information, and performing
  inference using TrueSkill$^{TM}$ \cite{HerbrichMG06} (an inference
  scheme based on expectation propagation \cite{Minka01}).
...
simulations\footnote{When experimenting with EUS and QI, the user is forced to select
its preffered item, so indifference is not possible. While support for indifference
could be added, we chose to be consistent with the methods as originally published.}, as in~\cite{guo2010real}.
